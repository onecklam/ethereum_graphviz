# -*- coding: utf-8 -*-
# wetapy: testskip
# Copyright (c) 2015, wetapy Development Team.
...
def _get_import_dir():
    ...
    if (op.isfile(op.join(up_dir, 'setup.py')) and
            op.isdir(op.join(up_dir, 'wetapy')) and
...
def _unit(mode, extra_arg_string, coverage=False):
    ...
    if mode == 'nobackend':
        ...
        if use_pytest:
            extra_args += ['-m', 'wetapy_app_test']
        else:
            extra_args += ['-a', 'wetapy_app_test']
    if coverage and use_pytest:
        extra_args += ['--cov', 'wetapy', '--no-cov-on-fail']
    ...
    if coverage:
        ...
        out_name = op.join(cwd, '.wetapy-coverage.%s' % mode)
...
def _docs():
    ...
    using wetapy/utils/tests/test_docstring_parameters.py"""
    dev = _get_import_dir()[1]

    if not dev:
        warnings.warn("Docstring test imports wetapy from"
                      " wetapy's installation. It is"
                      " recommended to setup wetapy using"
                      " 'python setup.py develop'"
                      " so that the latest sources are used automatically")
    try:
        # this should always be importable
        from wetapy.util.tests import test_docstring_parameters
...
def _flake():
    ...
    if dev:
        sys.argv[1:] = ['wetapy', 'examples', 'make']
    else:
...
def _examples(fnames_str):
    ...
    reason = None
    if not dev:
        reason = 'Cannot test examples unless in wetapy git directory'
    else:
    ...
    for fname in fnames:
        ...
        with open(fname, 'r') as fid:
            for _ in range(10):  # just check the first 10 lines
                ...
                if line == '':
                    break
                elif line.startswith('# wetapy: ') and 'testskip' in line:
                    good = False
                    break
        if not good:
            n_ran -= 1
...
def test(label='full', extra_arg_string='', coverage=False):
    ...
    if label == 'osmesa':
        ...
        # before the wetapy.gloo package gets imported
        from wetapy.util.osmesa_gl import fix_osmesa_gl_lib
        fix_osmesa_gl_lib()

    from wetapy.app.backends import BACKEND_NAMES as backend_names
    label = label.lower()
