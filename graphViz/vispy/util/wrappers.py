...
The idea of a 'global' wetapy.use function is that although wetapy.app
and wetapy.gloo.gl can be used independently, they are not complely
...
we probably want it to be easy to configure wetapy to use the ipython
...
def use(app=None, gl=None):
    """ Set the usage options for wetapy
    ...
    gl : str
        The gl backend to use (case insensitive). Options are:
            * 'gl2': use wetapy's desktop OpenGL API.
    ...
    Notes
    -----
    If the app option is given, ``wetapy.app.use_app()`` is called. If
    the gl option is given, ``wetapy.gloo.use_gl()`` is called.

    If an app backend name is provided, and that backend could not be
    loaded, an error is raised.

    If no backend name is provided, wetapy will first check if the GUI
    ...
    'default_backend' provided in the wetapy config. If still not
    succesful, it will try each backend in a predetermined order.

    See Also
    --------
    wetapy.app.use_app
    wetapy.gloo.gl.use_gl
    """
    ...
    if gl:
        import wetapy.gloo
        from wetapy import config
        config['gl_backend'] = gl
        wetapy.gloo.gl.use_gl(gl)
    if app:
        import wetapy.app
        wetapy.app.use_app(app)
