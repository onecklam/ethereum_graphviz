...
class AGraph(object):
    ...
    def layout(self, prog='neato', args=''):
        ...
        Optional prog=['neato'|'dot'|'twopi'|'circo'|'fdp'|'nop'|'sfdp']
        ...
        fmt = 'dot'
        if prog == "mingle":
            data = self._run_prog(prog, ' '.join([args]))
        else:
    ...
    def layout2(self, prog='neato', args=''):
        """Assign positions to nodes in graph.
    ...
    def _which(self, name):
        ...
        for path in paths.split(os.pathsep):
            ...
            if match:
                return match[0]'''
        #path = os.getcwd()+'\\bin'
        currentDir = os.path.join(os.path.abspath('.'), "bin")
        #currentDir = (os.path.split(os.path.realpath(__file__))[0])
        #print (currentDir)
        #currentDir = currentDir.replace('WetaAnalyse\\pygraphviz','WBin')
        #path = os.path.join(currentDir, "bin")
        #print (path)
        match = glob.glob(os.path.join(currentDir, name + exe))
        if match:
            return match[0]
